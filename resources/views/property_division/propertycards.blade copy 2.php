...
<body>
    ...
    <script>
        $(document).on("input", 'table tbody tr:last input[name="issue_qty"]', function() {
    var $row = $(this).closest("tr");
    var issueQty = $(this).val();
    console.log("THIS IS THE ISSUEQTY " + issueQty);
    var index = $row.index();
    if (index > 1 && index < $("table tbody tr").length) {
        var prevRow = $row.prev(); // Get the previous row
        var prevRowIssueTransferDisposal = prevRow.find('select[name="issue_transfer_disposal"]').val();
        if (prevRowIssueTransferDisposal === 'TRANSFER' || prevRowIssueTransferDisposal === 'DISPOSAL') {
            // Continue checking the row above the previous row until it finds ISSUE
            var foundIssue = false;
            var tempPrevRow = prevRow;
            while (tempPrevRow.length > 0 && !foundIssue) {
                var tempPrevRowIssueTransferDisposal = tempPrevRow.find('select[name="issue_transfer_disposal"]').val();
                if (tempPrevRowIssueTransferDisposal === 'ISSUE') {
                    foundIssue = true;
                    $.ajax({
                        url: "/get-latest-issue-row",
                        method: "GET",
                    ...
                    tempPrevRow = tempPrevRow.prev();
                }
            }
            if (!foundIssue) {
                console.log("No previous row with ISSUE value found.");
                return; // Exit the function if no ISSUE row is found
            }
        }
    }
});
